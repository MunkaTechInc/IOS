...
import SDWebImage
class MNFreelancerProflleVC: UIViewController,getStatusOfSwitch {
...
//                        ["image":"ic_withdraw","name":"Withdraw Amount"],
                        ...
                        ["image":"ic_earning","name":"My Earning"],
                        ["image":"ic_favourite","name":"Favorite Jobs"],
...
//                        ["image":"ic_refer_friend","name":"Refer Friends"],
...
//        imgProfile.sd_setImage(with: URL(string:img_BASE_URL + imge), placeholderImage:#imageLiteral(resourceName: "ic_individual"))
...
ShowHud(view: self.view)
                ...
                let status = response["status"] as! String
                 ...
                 if status == "1"
                 {
                   ...
                   // self.strNotification = response.n
                     print("self.viewDetails.stripeCustomerId = ",self.viewDetails.stripeCustomerId)
                     print("MyDefaults().StripeCustomerId = ",MyDefaults().StripeCustomerId)
    ...
    @IBAction func actionOnViewProfile(_ sender: UIButton) {
     let viewProfile = storyBoard.freelauncer.instantiateViewController(withIdentifier: "MNFreelancerViewProfileVC") as! MNFreelancerViewProfileVC
        ...
        Constant.isFreelancer = "freelancer"
...
// MARK: - extension
extension MNFreelancerProflleVC: UITableViewDataSource,UITableViewDelegate{
...
//               }
        if Tag == 0 {
        let myJob = storyBoard.freelauncer.instantiateViewController(withIdentifier: "MNFreelauncerMyJobsVC") as! MNFreelauncerMyJobsVC
        ...
        if Tag == 1 {
            let myJob = storyBoard.freelauncer.instantiateViewController(withIdentifier: "MNMyEarningVC") as! MNMyEarningVC
        ...
        if Tag == 2 {
               let myJob = storyBoard.freelauncer.instantiateViewController(withIdentifier: "MNFavouriteJobsVC") as! MNFavouriteJobsVC
            
              myJob.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(myJob, animated: true)
            }
//        if Tag == 3 {
//            let profilePassword = storyBoard.Individual.instantiateViewController(withIdentifier: "MNIndividualPromoCodeOfferVC") as! MNIndividualPromoCodeOfferVC
//            
...
//               self.navigationController?.pushViewController(profilePassword, animated: true)
           // self.NotificationOff(status: <#T##String#>)
        }
        if Tag == 6 {
            let profilePassword = storyBoard.Main.instantiateViewController(withIdentifier: "FAQViewController") as! FAQViewController
